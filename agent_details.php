<script>
	if (!localStorage.getItem('userInfo')) {
		window.location.href = 'https://91clubz.store/#/login';
	}
	else{
		...
		.then(data => {
			if(data.res != '1'){
				window.location.href = 'https://91clubz.store/';
			}
			else{
				let date = new Date();
      ...
      <div class="main-panel">
		...
		<footer class="footer">
			<div class="d-sm-flex justify-content-center justify-content-sm-between">
				<span class="text-muted d-block text-center text-sm-left d-sm-inline-block">Copyright © 150Rupya</span>
			</div>
		</footer>
      </div>     
    </div>
